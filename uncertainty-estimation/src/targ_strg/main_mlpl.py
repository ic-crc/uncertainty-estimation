...
def load_data_mlpl(root_data_dir):
    train_data = pd.read_parquet(root_data_dir+'mlpl_train.parquet', engine='pyarrow')
    cal_data = pd.read_parquet(root_data_dir+'mlpl_calibrate.parquet', engine='pyarrow')
    test_data = pd.read_parquet(root_data_dir+'mlpl_test.parquet', engine='pyarrow')

    #columns
    feat_cols = ['Frequency [MHz]', 'Link Distance [m]', 'Total Obstacle Depth [m]']
...
def proc_data(seed, data_refs, feat_cols):
    ...
    #columns
    target = 'Measured Path Loss [dB]'
    prediction = 'Predicted Path Loss [dB]'
    ...
    #df_train = df_train.sample(frac=0.0078125*2,random_state=seed).reset_index(drop=True)
    df_train = df_train.sample(frac=0.0078125/2,random_state=seed).reset_index(drop=True)
    train_size = df_train.shape[0]
    
    cal_size = round(train_size*(4/3)*(1/4))
    ...
    df_cal = df_cal.sample(n=cal_size,random_state=seed).reset_index(drop=True)

    test_size = train_size+cal_size
...
def conformal_prediction(data_refs,seed,knn_num,bin_num, targ_strangeness, feat_cols, req_conf_intervals):  
    ...
    df_results = cps_regression(seed, df_train, df_cal, df_test,feat_cols, knn_num,bin_num,targ_strangeness, req_conf_intervals)

    return df_results
###########
#main loop
def proc_and_plt_conformal_predictions(configs, seed_list):
    root_data_dir = '../../data/mlpl/'
    ...
    for config in configs:
        ...
        ray_results = [conformal_prediction.remote(data_refs, seed, knn_num, bin_num, targ_strangeness, \
                                                   feat_cols, req_conf_intervals=req_conf_intervals)
...
if __name__ == "__main__":
   '''
   example how to run
   /home/ray/cp_mm_targ_strg/src$ python main_mlpl.py --seed_list 56422 536073 9164 9847117 92631321
   '''
   
   parser = argparse.ArgumentParser(description="MLPL prediction intervals")
   ...
   configs = ['ext_num'] 
   proc_and_plt_conformal_predictions(configs,args.seed_list)
